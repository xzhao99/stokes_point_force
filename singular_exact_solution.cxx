...
namespace libMesh
{
  ...
  SingularExactSolution::SingularExactSolution(const EquationSystems& es) :
  ...
  _extra_order(0)
  {
    ...
    for (unsigned int sys=0; sys<_equation_systems.n_systems(); ++sys)
    {
      ...
      SingularExactSolution::SystemErrorMap sem;
    ...
    // initialize the flag
    _points_attached = false;
  }
  
  
  SingularExactSolution::~SingularExactSolution()
  {
  ...
  void SingularExactSolution::attach_reference_solution (const EquationSystems* es_fine)
  {
  ...
  void SingularExactSolution::attach_exact_value (Number fptr(const Point& p,
  ...
  void SingularExactSolution::attach_exact_values (std::vector<FunctionBase<Number> *> f)
  {
    // Clear out any previous _exact_values entries, then add a new
    // entry for each system.
    for (unsigned int i=0; i != _exact_values.size(); ++i)
  ...
  void SingularExactSolution::attach_exact_value (unsigned int sys_num,
  ...
  void SingularExactSolution::attach_exact_deriv (Gradient gptr(const Point& p,
                                                                const Parameters& parameters,
                                                                const std::string& sys_name,
                                                                const std::string& unknown_name))
  {
  ...
  void SingularExactSolution::attach_exact_derivs (std::vector<FunctionBase<Gradient> *> g)
  {
    // Clear out any previous _exact_derivs entries, then add a new
    // entry for each system.
    for (unsigned int i=0; i != _exact_derivs.size(); ++i)
  ...
  void SingularExactSolution::attach_exact_deriv (unsigned int sys_num,
  ...
  void SingularExactSolution::attach_exact_hessian (Tensor hptr(const Point& p,
                                                                const Parameters& parameters,
                                                                const std::string& sys_name,
                                                                const std::string& unknown_name))
  {
  ...
  void SingularExactSolution::attach_exact_hessians (std::vector<FunctionBase<Tensor> *> h)
  {
    // Clear out any previous _exact_hessians entries, then add a new
    // entry for each system.
    for (unsigned int i=0; i != _exact_hessians.size(); ++i)
  ...
  void SingularExactSolution::attach_exact_hessian (unsigned int sys_num,
  ...
  std::vector<Real>& SingularExactSolution::_check_inputs(const std::string& sys_name,
                                                          const std::string& unknown_name)
  {
  ...
  void SingularExactSolution::compute_error(const std::string& sys_name,
                                            const std::string& unknown_name)
  {
  ...
  Real SingularExactSolution::error_norm(const std::string& sys_name,
                                         const std::string& unknown_name,
  ...
  Real SingularExactSolution::l2_error(const std::string& sys_name,
                                       const std::string& unknown_name)
  {
  ...
  Real SingularExactSolution::l1_error(const std::string& sys_name,
                                       const std::string& unknown_name)
  {
  ...
  Real SingularExactSolution::l_inf_error(const std::string& sys_name,
                                          const std::string& unknown_name)
  {
  ...
  Real SingularExactSolution::h1_error(const std::string& sys_name,
                                       const std::string& unknown_name)
  {
  ...
  Real SingularExactSolution::hcurl_error(const std::string& sys_name,
                                          const std::string& unknown_name)
  {
  ...
  Real SingularExactSolution::hdiv_error(const std::string& sys_name,
                                         const std::string& unknown_name)
  {
  ...
  Real SingularExactSolution::h2_error(const std::string& sys_name,
                                       const std::string& unknown_name)
  {
  ...
  void SingularExactSolution::_compute_error(const std::string& sys_name,
                                             const std::string& unknown_name,
                                             std::vector<Real>& error_vals)
  {
    ...
    if( (n_vec_dim > 1) && _equation_systems_fine )
    {
      libMesh::err << "Error calculation using reference solution not yet\n"
                   << "supported for vector-valued elements." << std::endl;
      libmesh_not_implemented();
    }
    
    AutoPtr<QBase> qrule = fe_type.default_quadrature_rule (dim, _extra_order);
    ...
    // MeshFunction objects in each thread rather than a single master)
    ...
    for ( ; el != end_el; ++el)
    {
      ...
      // reinitialize the element-specific data for the current element
      fe->reinit (elem);
      ...
      /// ---------------------------------------------------------------------------
      // check: if distances between all the nodal points of the elem and the singular
      // points is smaller than the assigned radius, we exclude the error value
      // in this element!
      if(_points_attached)
      {
        bool inside_particle = false;
        
        // check if this element is inside the radius of particle
        for(unsigned int i=0; i<_singular_points.size(); ++i)
        {
          unsigned int k_pts = 0;
          for(unsigned int j=0; j<n_sf; ++j)
          {
            const Real dist = GeomTools::point_distance(elem->point(j), _singular_points[i]);
            if(dist<=_radius[i])
            {
              //std::cout<<"current quad-point is in the singular domain: "<<q_point[qp]<<std::endl;
              k_pts++;
            } // end if
          } // end for j-loop
        ...
        // if true, ignore this element for error estimation
        if(inside_particle)   continue;
      } //end if
      /// ---------------------------------------------------------------------------
      ...
      for (unsigned int qp=0; qp<n_qp; qp++)
      {
        ...
        // Compute solution values at the current quadrature point.
        // This reqiures a sum over all the shape functions evaluated
        ...
        if (_exact_values.size() > sys_num && _exact_values[sys_num])
        {
          for( unsigned int c = 0; c < n_vec_dim; c++)
            ...
            _exact_values[sys_num]->component(var_component+c, q_point[qp], time);
        }
    ...
    // Add up the error values on all processors, except for the L-infty norm, 
    // for which the maximum is computed.
  ...
  template void SingularExactSolution::_compute_error<Real>(const std::string&, const std::string&, std::vector<Real>&);
  template void SingularExactSolution::_compute_error<RealGradient>(const std::string&, const std::string&, std::vector<Real>&);
